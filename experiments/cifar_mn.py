...
dataloader = torch.utils.data.dataloader.DataLoader(dataset, batch_size=50000)
...
for i in range(32):
    for j in range(32):
        for k in range(3):
            random_variables.append(trv.RandomVariable(str((i,j,k)), list(range(11))))
        cliques.append(random_variables[-3:] + ["Label"])

random_variables.append(trv.RandomVariable("Label", list(range(10))))
model = MarkovNetwork(random_variables, cliques, verbose=1, device="cpu")
...
with torch.no_grad():
    for batch in dataloader:
        ...
        probability = probability.reshape(len(batch),10, -1).cpu().detach()
